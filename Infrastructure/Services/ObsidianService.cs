...
public sealed class ObsidianService : IObsidianService
{
    ...
    public async Task<Result<string>> SaveNoteAsync(string vaultPath, string fileName, string content, string[]? sourceLinks = null, CancellationToken cancellationToken = default)
    {
        try
        {
            if (!Directory.Exists(vaultPath))
    ...
    private static string CreateYamlFrontmatter(string[]? sourceLinks = null)
    {
